...
// ── NOTIFICATIONS ─────────────────────────────────────────────
const NOTIFS = [
  { title: "New User Registered", msg: "A new learner joined the platform", time: "2 min ago", read: false, type: "newUser" },
  { title: "Job Application", msg: "3 new applications for Frontend Dev", time: "1 hr ago", read: false, type: "jobApplication" },
  { title: "Payment Received", msg: "$149 payment received from learner", time: "3 hr ago", read: false, type: "payment" },
];
function renderNotifs() {
  try {
    ...
    const visibleNotifs = NOTIFS.filter(n => {
      const notifType = n.type || 'newUser'; // Default to newUser if no type
      return prefs[notifType] !== false; // Show by default, only hide if explicitly disabled
    });
    ...
    $("notif-list").innerHTML = visibleNotifs.length ? visibleNotifs.map((n, i) => {
      const actualIndex = NOTIFS.indexOf(n);
      return `
      <div class="notif-item" data-notif="${actualIndex}">
        <div class="notif-row">
          ...
          <button class="notif-delete-btn" data-notif-id="${actualIndex}" style="background:none;border:none;cursor:pointer;padding:4px;color:var(--muted);hover:color:var(--danger);font-size:18px;line-height:1;">
    ...
    }).join("") : `<div style="text-align:center;padding:24px;color:var(--muted);font-size:13px;">${t('noNotifications')}</div>`;
    $("notif-list").querySelectorAll(".notif-item").forEach(el => {
      el.addEventListener("click", e => { 
        if (!e.target.closest('.notif-delete-btn')) {
          NOTIFS[+el.dataset.notif].read = true; 
          renderNotifs(); 
        }
      });
    });
...
if (markReadBtn) {
  markReadBtn.addEventListener("click", () => { NOTIFS.forEach(n => n.read = true); renderNotifs(); });
}
...
function initNotificationPreferences() {
  ...
  container.innerHTML = notificationTypes.map((type) => `
    <div style="display:flex;align-items:flex-start;justify-content:space-between;padding:16px;background:var(--bg);border-radius:10px;border:1px solid var(--border);margin-bottom:12px;">
      <div style="flex:1;">
        <div style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:4px;">${type.title}</div>
        <div style="font-size:13px;color:var(--muted);">${type.desc}</div>
      </div>
