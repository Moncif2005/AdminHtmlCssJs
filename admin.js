...
function refreshStaticUI() {
  ...
  const titles = {
    dashboard: t('dashboard'), users: t('userManagement'), courses: t('courses'),
    certificates: t('certificates'), jobs: t('jobOffers'), applications: t('nav_applications'),
    payments: t('payments'), statistics: t('statistics'), settings: t('settings')
  };
  if (currentPage) $('header-title').textContent = titles[currentPage] || currentPage;
}
...
function navigate(page) {
  ...
  const titles = {
    ...
    jobs: t('jobOffers'),
    applications: t('nav_applications'),
    payments: t('payments'),
...
function removeUserFromCache(userId) {
  _usersCache = _usersCache.filter(u => u.id !== userId);
  if ($("users-table-wrap")) renderFilteredUsers();
}

function updateUserInCache(userId, updates) {
  _usersCache = _usersCache.map(u => u.id === userId ? { ...u, ...updates } : u);
  if ($("users-table-wrap")) renderFilteredUsers();
}
...
function removeCourseFromCache(courseId) {
  _coursesCache = _coursesCache.filter(c => c.id !== courseId);
  if ($("courses-table-wrap")) renderFilteredCourses();
}

function updateCourseInCache(courseId, updates) {
  _coursesCache = _coursesCache.map(c => c.id === courseId ? { ...c, ...updates } : c);
  if ($("courses-table-wrap")) renderFilteredCourses();
}
...
function normalizeApplicationStatus(status) {
  ...
  if (status === "reviewing" || status === "interview") return "pending";
...
$("confirm-delete-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (pendingDeleteType === "user") {
      removeUserFromCache(pendingDeleteId);
    }
    if (pendingDeleteType === "course") {
      removeCourseFromCache(pendingDeleteId);
    }
    showToast(t('deletedSuccessfully'), "success");
    if (reloads[pendingDeleteType] && pendingDeleteType !== "user" && pendingDeleteType !== "course") await reloads[pendingDeleteType]();
  } catch (err) {
...
function renderUsersTable(users) {
  ...
  wrap.innerHTML = `<div class="table-wrap"><table>
    ...
    ${users.map(u => `<tr>
      ...
      <td><div class="action-btns">
        <button class="tbl-btn" data-action="edit-user" data-id="${u.id}" data-name="${escapeAttr(u.name || u.displayName || "")}" data-email="${escapeAttr(u.email || "")}" data-role="${escapeAttr(u.role || "")}" data-phone="${escapeAttr(u.phone || "")}">
        ...
        <button class="tbl-btn danger" data-action="del-user" data-id="${u.id}" data-name="${escapeAttr(u.name || u.displayName || u.email || "")}">
  ...
  wrap.onclick = e => {
    ...
    if (!btn) return;
    if (btn.dataset.action === "edit-user") return editUser(btn.dataset.id, btn.dataset.name, btn.dataset.email, btn.dataset.role, btn.dataset.phone);
    if (btn.dataset.action === "del-user") return showConfirm(`Delete "${btn.dataset.name}"? This cannot be undone.`, btn.dataset.id, "user");
  };
}
...
$("user-save-btn").addEventListener("click", async () => {
  ...
  if (!userId) { showToast("Select a user to edit first.", "error"); return; }
  ...
  try {
    await updateDoc(doc(db, "users", userId), { name, email, role, phone });
    updateUserInCache(userId, { name, email, role, phone });
...
function renderCoursesTable(courses) {
  ...
  wrap.innerHTML = `<div class="table-wrap"><table>
    ...
    ${courses.map(c => `<tr>
      ...
      <td><div class="action-btns">
        <button class="tbl-btn" data-action="edit-course" data-id="${c.id}" data-name="${escapeAttr(c.name || "")}" data-instructor="${escapeAttr(c.instructor || "")}" data-type="${escapeAttr(c.type || "")}" data-price="${c.price}" data-desc="${escapeAttr(c.description || "")}" data-category="${escapeAttr(c.category || "")}">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>
        </button>
        <button class="tbl-btn danger" data-action="del-course" data-id="${c.id}" data-name="${escapeAttr(c.name || "")}">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
        </button>
      </div></td>
    </tr>`).join("")}
  ...
  wrap.onclick = e => {
    const btn = e.target.closest("[data-action]");
    if (!btn) return;
    if (btn.dataset.action === "edit-course") return editCourse(btn.dataset.id, btn.dataset.name, btn.dataset.instructor, btn.dataset.type, btn.dataset.price, btn.dataset.desc, btn.dataset.category);
    if (btn.dataset.action === "del-course") return showConfirm(`Delete course "${btn.dataset.name}"?`, btn.dataset.id, "course");
  };
}
...
$("course-save-btn").addEventListener("click", async () => {
  ...
  if (!courseId) { showToast("Select a course to edit first.", "error"); return; }
  const data = {
...
async function loadApplications(filterOfferId = null, filterOfferTitle = null) {
  ...
  try {
    ...
    const enriched = await Promise.all(snap.docs.map(async d => {
      ...
      return { id: d.id, ...data, status: normalizeApplicationStatus(data.status), applicantName, applicantEmail };
    }));
    ...
    $("app-stats").innerHTML = `
      ...
      <div class="stat-card"><div class="stat-card-top"><div class="stat-icon" style="background:linear-gradient(135deg,var(--success),#16a34a)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg></div></div><div class="stat-label">Approved</div><div class="stat-value">${counts.approved}</div></div>
      <div class="stat-card"><div class="stat-card-top"><div class="stat-icon" style="background:linear-gradient(135deg,var(--danger),#dc2626)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div></div><div class="stat-label">Refused</div><div class="stat-value">${counts.refused}</div></div>
      <div class="stat-card"><div class="stat-card-top"><div class="stat-icon" style="background:linear-gradient(135deg,#3b82f6,#6b63ff)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div></div><div class="stat-label">Total</div><div class="stat-value">${enriched.length}</div></div>`;
...
function renderApplicationsTable(apps) {
    ...
    ${apps.map(a => `<tr>
      ...
      <td>
        <div class="action-btns" style="flex-wrap:wrap;gap:4px;">
          ${a.status !== 'approved' ? `
            <button class="tbl-btn" style="color:var(--success);" title="Approve Application" data-action="app-approve" data-id="${a.id}" data-uid="${a.applicantId}" data-offer="${(a.offerTitle||'').replace(/"/g,'')}" data-company="${(a.company||'').replace(/"/g,'')}">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
            </button>` : ''}
          ${a.status !== 'refused' ? `
            <button class="tbl-btn danger" title="Refuse Application" data-action="app-refuse" data-id="${a.id}" data-uid="${a.applicantId}" data-offer="${(a.offerTitle||'').replace(/"/g,'')}" data-company="${(a.company||'').replace(/"/g,'')}">
  ...
  // Action listeners
  wrap.querySelectorAll("[data-action='app-approve']").forEach(btn => {
    btn.addEventListener("click", () => {
      if (confirm(`Approve this application for "${btn.dataset.offer}"? The candidate will be notified immediately.`)) {
        updateApplicationStatus(btn.dataset.id, btn.dataset.uid, 'approved', btn.dataset.offer, btn.dataset.company);
      }
    });
  });
  wrap.querySelectorAll("[data-action='app-refuse']").forEach(btn => {
    btn.addEventListener("click", () => {
      if (confirm(`Refuse this application for "${btn.dataset.offer}"? The candidate will be notified.`)) {
        updateApplicationStatus(btn.dataset.id, btn.dataset.uid, 'refused', btn.dataset.offer, btn.dataset.company);
      }
    });
  });
}
...
async function renderStatistics() {
  ...
  try {
    ...
    const appStatus = { pending: 0, approved: 0, refused: 0 };
    appsSnap?.docs.forEach(d => {
      const s = normalizeApplicationStatus(d.data().status);
      if (appStatus[s] !== undefined) appStatus[s]++;
    });
      ...
      <div class="grid-2" style="margin-bottom:20px;">
        ...
        <div class="card">
          ...
          ${totalApps ? Object.entries(appStatus).map(([status, count]) => {
            ...
            const colors = { pending: "#f97316", approved: "#22c55e", refused: "#ef4444" };
